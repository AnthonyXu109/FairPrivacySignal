...
def _policy_recovery_table(summary: pd.DataFrame) -> str:
    selected = summary[
    ...
    scenario_names = {
        "severe_signal_loss": "Complete signal loss",
    ...
    scenario_order = {
        "severe_signal_loss": 0,
        "policy_restricted": 1,
    }
    method_order = {
        "no_recovery": 0,
        "flat_privacy_safe_aggregates": 1,
        "policy_aware_signal_recovery": 2,
    }
    selected["scenario_order"] = selected["scenario"].map(scenario_order)
    selected["method_order"] = selected["variant"].map(method_order)
    rows = []

    for _, row in selected.sort_values(["scenario_order", "method_order"]).iterrows():
        rows.append(
            {
                "Regime": scenario_names[row["scenario"]],
                "Method": method_names[row["variant"]],
                "Overall NDCG@3": _mean_std(row, "overall_ndcg_at_3"),
                "Low-signal NDCG@3": _mean_std(row, "low_signal_ndcg_at_3"),
                "Full-signal gap closed": (
                    f"{row['overall_gap_closed_mean']:.1%}"
                    if row["variant"] != "no_recovery"
                    else "-"
                ),
            }
        )
...
def build_benchmark_card(
    ...
    coverage_rows = [
        {
            "Diagnostic": "Primary signal-recovery method",
            "Coverage": (
                f"{policy_aware_recovery_summary['scenario'].nunique()} policy regimes; "
                f"{policy_aware_recovery_summary['variant'].nunique()} method variants"
            ),
        },
        {
    ...
    return (
        "# FairPrivacySignal Method and Benchmark Card\n\n"
        "This reviewer-facing card is generated by the reproducible benchmark "
        "pipeline from auditable synthetic-data outputs. It summarizes the primary "
        "recovery method and its experimental audit surface without replacing the "
        ...
        + "\n\n"
        "## Primary Signal-Recovery Method\n\n"
        "Policy-aware recovery chooses a stable aggregate path under complete signal "
        "loss and fuses cross-fitted signal reconstruction with aggregates when some "
        "individual signal remains policy-permitted.\n\n"
        + _policy_recovery_table(policy_aware_recovery_summary)
        + "\n\n"
        ...
        "- [Policy-aware signal recovery](policy_aware_signal_recovery.md)\n"
        ...
        "FairPrivacySignal is a synthetic research prototype. It does not model "
...
def main() -> None:
    ...
    card = build_benchmark_card(
        ...
        policy_aware_recovery_summary=pd.read_csv(
            tables_dir / "policy_aware_signal_recovery_summary.csv"
        ),
